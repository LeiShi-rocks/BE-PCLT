...
factor.comb <- function(point.estimates, covariance.est, num.ties, 
  ...
  for (ind.tie in 1:num.ties){
    ...
    qtail <- qnorm(alpha/2, mean = 0, sd = 1, lower.tail = F)
...
comb.select.trial <- function(factorial.data.init, alpha.vec, level, 
  ...
  for (iter in 1:Niter){
    # first six methods
    factorial.data <- factorial.data(num_factors, trt_group_size, pop = pop,
    ...
    for (method.ind in 1:8){
      ...
      if (running.flag && method.ind <= 6){
        ...
        simulation.res$model.select.res[[iter]][[running.tag]] <- selected.model.output$selected_model
        
        
        # factor level combination selection
        selected.model <- selected.model.output$selected_model
        post.design.run <- dplyr::select(design.run, selected.model)
        post.design.core <- dplyr::select(design.core, selected.model)
      ...
      if (running.flag && method.ind == 7){ # Naive Bonferroni
        ...
        select.model.output <- model.selection(running.factorial_data, 
                                               ...
                                               wt = wt,
                                               method = "Bonferroni")
        select.model.output <- select.model.output$post_working_model
        simulation.res$model.select.res[[iter]][[running.tag]] <- select.model.output
        
        # factor level combination selection
        selected.model <- select.model.output
        post.design.run <- dplyr::select(design.run, selected.model)
        post.design.core <- dplyr::select(design.core, selected.model)
        post.factorial.data <- cbind(y = factorial_data$y, post.design.run)
        lm.post <- lm(y~., data = post.factorial.data)
