...
tau <- c(
  ...
  runif(choose(K,2), 0.1, 0.5) * sign(runif(choose(K,2), -1, 1)) * rbinom(choose(K,2), 1, 0.7)
)
...
```{r}
# report results
record <- readRDS("record_SMALL_EFFECTS.RData")
# point estimates
...
plot_SE <- ggplot(hist_data, aes(x = labs, y = est, fill = labs)) + 
  geom_violin() + 
...
tau <- c(
  runif(choose(K,0), 0.1, 0.5) * sign(runif(choose(K,0), -1, 1)),
  runif(choose(K,1), 0.5, 1.0) * sign(runif(choose(K,1), -1, 1)),
  runif(choose(K,2), 0.1, 0.5) * sign(runif(choose(K,2), -1, 1)) * rbinom(choose(K,2), 1, 0.7)
)
...
```{r}
# report results
record <- readRDS("record_LARGE_EFFECTS.RData")
# point estimates
hist_data <- data.frame(
  est = c(t(record$rec_point_est)),
  labs = factor(rep(target_effect, each = 1000), levels = c('F2', 'F4', 'F6', 'F8', 'F10'))
...
plot_LE <- ggplot(hist_data, aes(x = labs, y = est, fill = labs)) + 
  geom_violin() + 
  theme(legend.position="none",
        text = element_text(size = 20)) +
  scale_x_discrete(labels = c(
