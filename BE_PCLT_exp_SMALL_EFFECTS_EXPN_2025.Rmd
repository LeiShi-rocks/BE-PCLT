...
K <- 7
num_trt_group <- 2^K
Q_U <- floor(0.65 * 2^K) # unreplicated small arms
Q_R <- floor(0.33 * 2^K) # replicated small arms
Q_L <- 2^K - Q_U - Q_R # large arms
...
trt_group_size[(Q_U + Q_R + 1):(2^K)] <- 30
...
# add id for each arm
id_arm <- factorial_data %>% group_by(across((K+1):2)) %>%
  group_indices()
...
data_fit <- lm(y~-1+F1+F2+F3+F4+F5, data = data_in, weights = rep(1/trt_group_size, trt_group_size))
summary(data_fit)
...
# Alternatively, do unsaturated regression - weighted least square + EHW variance estimation
data_in <- data.frame(y=factorial_data$y, design.run)
data_fit <- lm(y~-1 + (F1 + F2 + F3 + F4 + F5)^2, data = data_in, weights = rep(1/trt_group_size, trt_group_size))
summary(data_fit)
...
```{r}
saveRDS(results, file = "Revision-202505/sim_results_parallel.rds")
```
...
```{r}
# report results
record <- readRDS("Revision-202505/sim_results_parallel.rds")
```
...
hist_data <- data.frame(
  ...
  labs = factor(rep(target_effect, each = 1000), levels = c('F1', 'F2', 'F3', 'F4', 'F5'))
)

summary_data <- data.frame(
  target_tau = target_tau,
  labs = factor(target_effect, levels = c('F1', 'F2', 'F3', 'F4', 'F5'))
)
...
saveRDS(record, file="Revision-202505/record_SMALL_EFFECTS_EXPN.RData")
